...
export function ArrivalChart({ data, moyenne }: ArrivalChartProps) {
  ...
  const chartData = data.map(point => ({
    ...point,
    heureMinutes: point.heure_debut ? convertTimeToMinutes(point.heure_debut) : 0,
  }));
    ...
    <ResponsiveContainer width="100%" height={300}>
      <LineChart data={chartData}>
        ...
        <Tooltip
          ...
          formatter={(value: number) => [convertMinutesToTime(value), 'Heure d\'arrivée']}
        />
        <Legend />
        <Line
          type="monotone"
